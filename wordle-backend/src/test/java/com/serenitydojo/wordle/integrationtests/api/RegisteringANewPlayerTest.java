...
public class RegisteringANewPlayerTest {
    ...
    void registeringAsANewPlayer() {
        ...
        Player player = new Player(name, password, email,"US",false);
        SerenityRest
                .with()
                .body(player)
    ...
    void registeringAsANewPlayerWithAnExistingEmail() {
        ...
        SerenityRest
                .with()
                .body(new Player(email, password, name + " 1","GB",false))
                .contentType(ContentType.JSON)
                .post("/wordle/api/auth/register");

        SerenityRest
                .with()
                .body(new Player(email, password, name + " 2","GB",false))
                .contentType(ContentType.JSON)
                .post("/wordle/api/auth/register")
                .then().statusCode(409);
    }
    ...
    void registeringAsANewPlayerWithAnExistingUsername() {
        ...
        SerenityRest
                .with()
                .body(new Player("a1" + email, password, name,"GB",false))
                .contentType(ContentType.JSON)
                .post("/wordle/api/auth/register");

        SerenityRest
                .with()
                .body(new Player("a2" + email, password, name,"GB",false))
                .contentType(ContentType.JSON)
                .post("/wordle/api/auth/register")
                .then().statusCode(409);
    }
